...
* Initial state:
    ...
    In the case of a pure Pauli stat, the 'init_pauli_state' parameter should be used, in the case of a graph state, the 'init_graph_state' parameter should be used and in the case of a saved internal state the 'load_files_prefix' parameter should be used. For each simulation, only one of those parameters should be used and the others should be left empty or at their default value.
