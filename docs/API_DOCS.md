...
* Initial state:
    The initial state of the simulator is either defined by the parameters, or loaded from files saved by a previous solver execution. A state defined by the parameters can be initialized to a product state with each qubit in a pure Pauli state or a diagonal mixed state, and then possibly a graph state defined on some of the qubits by specifying a set <img src="https://render.githubusercontent.com/render/math?math=V" style="vertical-align:bottom"> of pairs of qubits, to all of which a controlled-Z gate is applied (after initializing all qubits along the positive <img src="https://render.githubusercontent.com/render/math?math=x" style="vertical-align:bottom"> axis). 
    In the case of a product of Pauli states or diagonal mixed states, the 'init_pauli_state' parameter should be used. For initializing a graph state (on all or part of the qubits), the 'init_graph_state' parameter should be used and in the case of a saved internal state the 'load_files_prefix' parameter should be used. If 'load_files_prefix' is used, the other two should be left empty or at their default value.

    * init_pauli_state = "+z" (list[str]): A length-N vector of either two-character strings of the form <img src="https://render.githubusercontent.com/render/math?math=\pm a" style="vertical-align:bottom">, or a float. Each string indicates the initial state of qubit <img src="https://render.githubusercontent.com/render/math?math=i" style="vertical-align:bottom">. The initial state will be
      ...
      If an entry is a float, it should be between 0 and 1, and indicates the population of the "|up>" state (aligned with +z) in a diagonal mixed-state density matrix for the qubit.
