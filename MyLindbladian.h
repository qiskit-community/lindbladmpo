...
void SetLindbladian(SpinHalfSystem &C, ModelParameters param, Lattice2d L)
{
    ...
    if (J.size() != 1 && J.size() != N)
        cerr << "Error: the paramter J has " << J.size() << " values  1 or " << N << " value(s) were expected.\n", exit(1);
    if (J.size() == 1)
    {
        J = vector<double>(N, J[0]);
    }
    vector<double> J_z = param.doublevec("J_z");
    if (J_z.size() != 1 && J_z.size() != N)
        cerr << "Error: the paramter J has " << J_z.size() << " values  1 or " << N << " value(s) were expected.\n", exit(1);
    if (J_z.size() == 1)
    {
        J_z = vector<double>(N, J_z[0]);
    }
