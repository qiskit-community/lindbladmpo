...
The problem is simulated in the frame rotating with the driven qubit's frequency, with the qubits
initialized along the z axis of the Bloch sphere (the pure product ground state), and subject to
...
tau = 0.01
# tau is the discrete integration (Trotterization) time step size.
# It should be significantly smaller than the period of all coherent dynamics.
max_dim_rho = 60
# This is the maximal bond dimension at each qubit.
...
h_x_amp = 0.5
# The center qubit in the chain has an x-axis term in the Hamiltonian with this amplitude (* 2 * pi).
