...
void ApplyControlledXYZGateONPureState(MPS &psi, const SpinHalf &sites, int control, int target, string opname,
                                       Args args)
{
    if (control == target)
        cerr << "Error, ApplyControlledXYZGateONPureState was called with control=target=" << control << ".\n", exit(1);
    ...
    const int N = length(psi);
    {
        ...
        for (int n = 1; n <= N; ++n)
        {
            ...
            if (n == 1)
            {
                ...
                else if (n == target)
                {
                    ...
                    W += 2 * sites.op(opname, n) *
                         setElt(
                             right(2)); // Apply 2*"opname" to the target bit if |control> = |1>. The 2* comes from the
                                        // fact that "Sx" is 0.5*Pauli^x when applied to a Spin-1/2 state (whereas it is
                                        // equal to 1.0*Pauli^x when applied on au mixed/Pauli/density matrix state)
                }
                else
            ...
            if (n > 1 && n < N)
            {
                ...
                else if (n == i && n == target)
                {
                    ...
                    W += 2 * sites.op(opname, n) * setElt(left(1)) *
                         setElt(right(2)); // Apply 2*opname to the target bit if |control> = |1>
                }
                else if (n > i && n < j)
                {
                ...
                {
                    W += sites.op("Id", n) * setElt(left(1)) * setElt(right(1));
                    W += 2 * sites.op(opname, n) * setElt(left(2)) *
                         setElt(right(1)); // Apply 2*opname to target bit if |control> = |1>
                }
                else
                    W += sites.op("Id", n) * setElt(right(1)) * setElt(left(1));
            }
        ...
        psi = applyMPO(gate_mpo, psi, args);
        psi.noPrime("Site");
    }
}
...
{
    if (control == target)
        cerr << "Error, ApplyControlledXYZGate was called with control=target=" << control << ".\n", exit(1);
    const int i = min(target, control), j = max(target, control);
    ...
    for (int braket = 0; braket <= 1; braket++)
    {
        ...
        for (int n = 1; n <= N; ++n)
        {
            ...
            if (n > 1 && n < N)
            {
                ...
                else if (n == i && n == target)
                {
                    ...
                    W +=
                        siteops.op(s, n) * setElt(left(1)) * setElt(right(2)); // Flip the target bit if |control> = |1>
                }
                else if (n > i && n < j)
                {
                ...
                else if (n == j && n == target)
                {
                    W += siteops.op("Id", n) * setElt(left(1)) * setElt(right(1));
                    W +=
                        siteops.op(s, n) * setElt(left(2)) * setElt(right(1)); // Flip the target bit if |control> = |1>
                }
                else
                    W += siteops.op("Id", n) * setElt(right(1)) * setElt(left(1));
            }
        ...
        rho = applyMPO(gate_mpo, rho, args);
        rho.noPrime("Site");
    }
}
...
void ApplyListOfGatesOnAPureState(string s, MPS &psi, const SpinHalfSystem &C)
{
    ...
    for (auto &op : ops)
    {
        ...
        case 2:
            ...
            i = stoi(st[1]);
            if (i < 1 || i > C.N)
                cout2 << "Error in SpinHalfSystem::ConstructProjectorFromGates: qubit index i=" << i
                      << " is out of range.\n",
                    exit(0);
            if (op_name == "X" || op_name == "x")
                psi.ref(i) *= 2 * C.sites.op("Sx", i), psi.ref(i).noPrime();
            else if (op_name == "Y" || op_name == "y")
                psi.ref(i) *= 2 * C.sites.op("Sy", i), psi.ref(i).noPrime();
            else if (op_name == "Z" || op_name == "z")
                psi.ref(i) *= 2 * C.sites.op("Sz", i), psi.ref(i).noPrime();
            else if (op_name == "H" || op_name == "h")
                psi.ref(i) *= Hadamard(C.sites, i), psi.ref(i).noPrime();
            else if (op_name == "U" || op_name == "u")
                psi.ref(i) *= C.sites.op("projUp", i), psi.ref(i).noPrime();
            else if (op_name == "D" || op_name == "d")
                psi.ref(i) *= C.sites.op("projDn", i), psi.ref(i).noPrime();
            // TODO: Verify whether the projectors need the factor of 2 ?
            else
                cout2 << "Error in SpinHalfSystem::ConstructProjectorFromGates: unknown 1-qubit operator " << op_name
                      << ".\n",
                    exit(0);
            break;
        case 3:
            ...
            i = stoi(st[1]);
            j = stoi(st[2]);
            if (op_name == "CX" || op_name == "cx")
                ApplyControlledXYZGateONPureState(psi, C.sites, i, j, "Sx", Args("Cutoff", 0));
            else if (op_name == "CNOT" || op_name == "cnot")
                ApplyControlledXYZGateONPureState(psi, C.sites, i, j, "Sx", Args("Cutoff", 0));
            else if (op_name == "CY" || op_name == "cy")
                ApplyControlledXYZGateONPureState(psi, C.sites, i, j, "Sy", Args("Cutoff", 0));
            else if (op_name == "CZ" || op_name == "cz")
                ApplyControlledXYZGateONPureState(psi, C.sites, i, j, "Sz", Args("Cutoff", 0));
            else
                cout2 << "Error in SpinHalfSystem::ConstructProjectorFromGates: unknown 2-qubit gate " << op_name
                      << ".\n",
                    exit(0);
            break;
        default:
            cout2 << "Error in SpinHalfSystem::ConstructProjectorFromGates: expecting an operator name followed by 1 "
                     "or 2 qubit number but got "
                  << op << ".\n",
                exit(0);
        }
    }
...
void StringToOperatorsList(string s, vector<string> &ops, vector<int> &qubits)
{
    ...
    for (auto &op : l_ops)
    {
        ...
        default:
            cout2 << "Error in StringToOperatorsList: expecting an operator name followed by 1 qubit number but got "
                  << op << ".\n",
                exit(0);
        }
    }
