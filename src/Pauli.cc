...
// Constructor
SpinHalfSystem::SpinHalfSystem(int size)
    : N(size), sites(N, {"ConserveQNs=", false}), siteops(N), rho(siteops), Lindbladian(siteops)
{
}
...
void SpinHalfSystem::psi2rho(const MPS &psi, MPS &RHO, const Args &args)
{
    ...
    int N = psi.length();
    if (RHO.length() != N)
        cout2 << "Error in psi2rho: psi has lenght " << N << " and RHO has lenghth " << RHO.length() << "\n.", exit(0);
...
Cplx SpinHalfSystem::Expect(const string &opname1, int i1, const string &opname2, int i2, const string &opname3,
                            int i3) const
{
    MPS rho_tmp(rho); // Could we avoid making this copy ?
    const auto &Op1 = siteops.op(opname1, i1);
...
{
    if (opnames.size() != indices.size())
        cout2 << "Error in SpinHalfSystem::Expect, openames and indices should have the same size.\n", exit(1);
    MPS rho_tmp(rho); // Could we avoid making this copy ?
...
void SpinHalfSystem::AddSingleSpinBath(double GammaPlus, double GammaMinus, double GammaDephasing, double GammaBitFlip,
                                       double GammaBitPhaseFlip, int site)
{
    if (GammaMinus)
    {
