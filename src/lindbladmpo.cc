...
int main(int argc, char *argv[])
{
    ...
    if (init_cz_gates.size() % 2 == 1)
        cout2 << "Error: the list of indices given in the parameter " << s_cz_param << " should have an even length.\n",
            exit(1);
    ...
    else
    {
        ...
        if (init_len == 1)
            ...
            cout2 << "Error: the parameter " << s_init_param << " has " << init_len << " value(s) but 0, 1 or " << N
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (s_type == "p ")
                    {
                        ...
                        {
                            cout2 << "Error: " << s_init
                                  << " is an unknown 1-qubit initial state. "
                                     "After the prefix \"p \" should follow a valid float value.\n",
                                exit(1);
                        }
                        if (b < 0. || b > 1.)
                            cout2 << "Error: " << s_val
                                  << " is an unknown 1-qubit initial "
                ...
                else
                {
                    ...
                    for (int r = 1; r <= dr; r++)
                        for (int l = 1; l <= dl; l++)
                        {
                            ...
                            C.rho.ref(site_number)
                                .set(pauli_ind = 2, li = l, ri = r, complex<double>(d_r, -d_i) * trace); // |d><u|
                            C.rho.ref(site_number)
                                .set(pauli_ind = 3, li = l, ri = r, complex<double>(d_r, d_i) * trace);  // |u><d|
    ...
    for (unsigned int n = 0; n < apply_gates.size(); n++)
    {
        ...
        if ((time - t_0) < (-.1 * tau) || (time - t_f) > .1 * tau)
            cout2 << "Error: time " << vs[0]
                  << " defined in parameter apply_gates is not between t_init and t_final.\n",
                exit(1);
        //		if (fmod(abs(time / tau), 1.) > 0.1 && fmod(abs(time / tau), 1.) < 0.9)
        //			cout2 << "Error: time " << vs[0] << " defined in parameter apply_gates is not close to an integer
        //multiple of tau.\n", exit(1);
        ...
        if (sgate == "X" || sgate == "Y" || sgate == "Z" || sgate == "SX" || sgate == "H" || sgate == "U" ||
            sgate == "D")
        {
            try
            {
                i = stod(vs[2]);
                if (i < 1 || i > N)
                    cout2 << "Error: qubit index " << i << " out of range in '" << apply_gates[n] << "'.\n", exit(1);
            }
            catch (...)
            {
                cout2 << "Error: " << vs[2] << " is not a valid integer (expecting a qubit number) in '"
                      << apply_gates[n] << "'.\n",
                    exit(1);
            }
            if (vs.size() == 4)
        ...
        else if (sgate == "CZ" || sgate == "CX")
        {
            ...
            try
            {
                j = stod(vs[3]);
                if (j < 1 || j > N)
                    cout2 << "Error: qubit index " << j << " out of range in '" << apply_gates[n] << "'.\n", exit(1);
            }
            catch (...)
            {
                cout2 << "Error: " << vs[3] << " is not a valid integer (expecting a qubit number) in '"
                      << apply_gates[n] << "'.\n",
                    exit(1);
            }
            gate_times.push_back(time);
    ...
    for (string c_obs : custom_obs)
    {
        ...
        if (obs_head[1] == "g")
        {
            ...
            for (int i = 1; i <= N; ++i)
                psi0_ini.set(i, "Up"); // Start with all spins up
    ...
    if (components3.size())
    {
        ...
        for (auto &s : components3)
        {
            if (s.length() != 3)
                cout2 << "Error: " << s
                      << " is an unknown 3-qubit component (should be a triplet in (x,y,z)*(x,y,z)*(x,y,z)).\n",
                    exit(1);
            for (int n = 0; n <= 2; n++)
            {
                char c = toupper(s[n]);
                if (c != 'X' && c != 'Y' && c != 'Z')
                    cout2 << "Error: " << s
                          << " is an unknown component (should be a triplet in (x,y,z)*(x,y,z)*(x,y,z)).\n",
                        exit(1);
            }
        }
    ...
    for (int n = 0; n <= n_steps; n++)
    {
        ...
        for (unsigned int k = 0; k < gate_times.size(); k++)
        {
            if (abs(gate_times[k] - t) < (tau / 2.))
            {
                ...
                else
                {
                    if (gate_names[k] == "U")
                    {
                        cout2 << "\tApplication of projector Up"
                              << "(" << gate_i[k] << ")\n";
                    ...
                    else
                    {
                        if (gate_names[k] == "D")
                        {
                            cout2 << "\tApplication of projector Down"
                                  << "(" << gate_i[k] << ")\n";
                        ...
                        else
                        {
                            ...
                            if (gate_names[k] == "X")
                                ApplyXGate(C.rho, C.siteops, gate_i[k]);
                            if (gate_names[k] == "Y")
                                ApplyYGate(C.rho, C.siteops, gate_i[k]);
                            if (gate_names[k] == "Z")
                                ApplyZGate(C.rho, C.siteops, gate_i[k]);
                            if (gate_names[k] == "SX")
                                ApplySqrtXGate(C.rho, C.siteops, gate_i[k]);
                            if (gate_names[k] == "H")
                                ApplyHGate(C.rho, C.siteops, gate_i[k]);
                        }
                    }
                }
                cout2.flush();
        ...
        if (output_step > 0)
        {
            if ((n % output_step) == 0 || n == n_steps)
            {
                ...
                //				file_global << t << " \t" << tr.real() << " \t" << S_2 << " \t" << osee << " \t" <<
                //bd_max << " \t" << tot_duration.count() << endl;
                file_global << t << " \t"
                            << "tr_rho\t" << tr.real() << "\n";
                file_global << t << " \t"
                            << "S_2\t" << S_2 << "\n";
                ...
                file_global << t << " \t"
                            << "max_bond_dim\t" << bd_max << "\n";
                file_global << t << " \t"
                            << "duration_ms\t" << tot_duration.count() << "\n";
                ...
                if (count)
                {
                    ...
                    cout2 << "\n\t" << count
                          << " 1-qubit expectation values saved to file. Duration: " << duration_ms.count() / 1000.
                          << "s";
                    count = 0;
                }
                // --------------------------------------------------
                ...
                if (count)
                {
                    ...
                    cout2 << "\n\t" << count
                          << " 2-qubit expectation values saved to file. Duration: " << duration_ms.count() / 1000.
                          << "s";
                    count = 0;
                }
                // --------------------------------------------------
                ...
                if (count)
                {
                    ...
                    cout2 << "\n\t" << count
                          << " 3-qubit expectation values saved to file. Duration: " << duration_ms.count() / 1000.
                          << "s";
                    count = 0;
                }
                ...
                if (count)
                {
                    ...
                    cout2 << "\n\t" << count
                          << " custom expectation values saved to file. Duration: " << duration_ms.count() / 1000.
                          << "s";
                    count = 0;
                }
                cout2 << "\n";
...
void validate_2q_list(vector<long> &vect, int N, string const &list_name)
{
    for (unsigned int n = 0; n < vect.size(); n += 2)
    {
        ...
        if (i == j)
            cout2 << "Error: an invalid identical index pair (" << i << ") found in list `" << list_name
                  << "`. Two-qubit operators must involve two distinct qubits.\n",
                exit(1);
    }
}
...
void validate_3q_list(vector<long> &vect, int N, string const &list_name)
{
    for (unsigned int n = 0; n < vect.size(); n += 3)
    {
        ...
        if (i == j || i == k || k == j)
            cout2 << "Error: invalid index triplet (" << i << "," << j << "," << k << ") found in list `" << list_name
                  << "`. 3-qubit operators must involve 3 distinct qubits.\n",
                exit(1);
    }
}
  ...
  if (param.longval("rho_inf_init") == 0)
  {
    ...
    { //Start from a wave-function (pure state)
      if (param.stringval("load_purestate_file") == "")
      {
        ...
        sweeps.maxdim() = min(5, m), min(5, m), min(10, m), min(10, m), min(10, m), min(20, m), min(20, m), min(50, m),
  min(50, m), min(100, m), min(100, m), min(200, m), min(200, m), m; sweeps.cutoff() = param.val("cut_off_psi");
